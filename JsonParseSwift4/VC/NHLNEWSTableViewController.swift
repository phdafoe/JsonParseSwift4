//
//  NHLNEWSTableViewController.swift
//  iCoNews
//
...
class NHLNEWSTableViewController: UITableViewController {
    
    //MARK: - Variables locales
    var arrayModelNHL : [ModelGeneralData] = []
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = arrayModelNHL[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let objInd = arrayModelNHL[selectInd!]
    ...
    func llamadaNHLNEWS(){
        ...
        let datosModelNHL = ParserGeneral()
        let idFuente = CONSTANTES.LLAMADAS.BASE_NHL_NEWS
        
        APESuperHUD.showOrUpdateHUD(loadingIndicator: .standard, message: "Cargando", presentingView: self.view)
        
        firstly{
            return when(resolved: datosModelNHL.getDatosFromWeb(idFuente))
            }.then{_ in
                self.arrayModelNHL = datosModelNHL.setParseFromWeb()
            }.then{_ in
